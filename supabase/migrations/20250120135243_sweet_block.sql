...
DO $$ BEGIN
  DROP POLICY IF EXISTS "Allow public read location_settings" ON location_settings;
  DROP POLICY IF EXISTS "Allow authenticated users manage location_settings" ON location_settings;
EXCEPTION
...
-- Insert default location if no records exist
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM location_settings LIMIT 1) THEN
